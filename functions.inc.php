...
function get_folders()
{
	...
	if ( !file_exists(DOCROOT.'/data/folders.db') )
		return $folders;
	
	$db = file(DOCROOT.'/data/folders.db');
...
function save_folders($folders)
{
	...
	file_put_contents(DOCROOT.'/data/folders.db', $result);
}
...
function get_list()
{
	...
	if ( !file_exists(DOCROOT.'/data/list.db') )
		return array();
	
	$db = file(DOCROOT.'/data/list.db');
	foreach($db as $row) {
		$row = explode("\t", trim($row));
...
function save_list($list)
{
	...
	file_put_contents(DOCROOT.'/data/list.db', $result);
}
